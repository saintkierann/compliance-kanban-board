...
export default function Kanban() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        try { await window.storage.set(STORAGE_KEY, JSON.stringify(DEFAULT_CARDS), false); } catch (e) {}
      }
      setLoading(false);
    })();
  }, []);
  ...
  const persist = (next) => {
    setCards(next);
    try { window.storage.set(STORAGE_KEY, JSON.stringify(next), false); } catch (e) {}
  };
  const move = (id, dir) => {
    let landed = null;
